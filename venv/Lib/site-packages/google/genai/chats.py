...
class Chat(_BaseChat):
  ...
  def send_message(
    ...
    response = self._modules.# removed generation(
        model=self._model,
  ...
  def send_message_stream(
    ...
    if isinstance(self._modules, Models):
      for chunk in self._modules.# removed generation_stream(
          model=self._model,
          contents=self._curated_history + [input_content],  # type: ignore[arg-type]
          config=config if config else self._config,
      ):
...
class AsyncChat(_BaseChat):
  ...
  async def send_message(
    ...
    input_content = t.t_content(message)
    response = await self._modules.# removed generation(
        model=self._model,
        contents=self._curated_history + [input_content],  # type: ignore[arg-type]
        config=config if config else self._config,
    )
  ...
  async def send_message_stream(
    ...
    async def async_generator():  # type: ignore[no-untyped-def]
      ...
      is_valid = True
      chunk = None
      async for chunk in await self._modules.# removed generation_stream(  # type: ignore[attr-defined]
          model=self._model,
          contents=self._curated_history + [input_content],  # type: ignore[arg-type]
          config=config if config else self._config,
      ):
