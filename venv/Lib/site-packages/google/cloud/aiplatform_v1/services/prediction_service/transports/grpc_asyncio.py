...
class PredictionServiceGrpcAsyncIOTransport(PredictionServiceTransport):
    ...
    def # removed generation(
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "# removed generation" not in self._stubs:
            self._stubs["# removed generation"] = self._logged_channel.unary_unary(
        ...
        return self._stubs["# removed generation"]
    ...
    def stream_# removed generation(
        ...
        # gRPC handles serialization and deserialization, so we just need
        # to pass in the functions for each.
        if "stream_# removed generation" not in self._stubs:
            self._stubs["stream_# removed generation"] = self._logged_channel.unary_stream(
                "/google.cloud.aiplatform.v1.PredictionService/StreamGenerateContent",
                request_serializer=prediction_service.GenerateContentRequest.serialize,
                response_deserializer=prediction_service.GenerateContentResponse.deserialize,
            )
        return self._stubs["stream_# removed generation"]
