...
async def google_# removed generation(
    request: Request,
    model_name: str,
    ...
    logging_obj, data = litellm.utils.function_setup(
        original_function="a# removed generation",
    ...
    response = await llm_router.a# removed generation(**data)
    return response
...
async def google_stream_# removed generation(
    request: Request,
    model_name: str,
    fastapi_response: Response,
    user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth),
    ...
    logging_obj, data = litellm.utils.function_setup(
        original_function="a# removed generation_stream",
        rules_obj=litellm.utils.Rules(),
        start_time=datetime.now(),
        **data,
    )
    ...
    if llm_router is None:
        raise HTTPException(status_code=500, detail="Router not initialized")
    response = await llm_router.a# removed generation_stream(**data)
...
async def google_count_tokens(request: Request, model_name: str):
    ...
    messages = (
        GoogleGenAIAdapter()
        .translate_# removed generation_to_completion(model_name, contents)
        .get("messages", [])
    )
