...
class GenerateContentToCompletionHandler:
    """Handler for transforming # removed generation calls to completion format when provider config is None"""
    ...
    def _prepare_completion_kwargs(
        ...
        # Transform # removed generation request to completion format
        completion_request = (
            GOOGLE_GENAI_ADAPTER.translate_# removed generation_to_completion(
                model=model,
                contents=contents,
                config=config,
    ...
    @staticmethod
    async def async_# removed generation_handler(
        model: str,
        contents: Union[List[Dict[str, Any]], Dict[str, Any]],
        ...
        **kwargs,
        ...
        """Handle # removed generation call asynchronously using completion adapter"""
        ...
        try:
            ...
            if stream:
                ...
                if not hasattr(completion_response, "__aiter__"):
                    ...
                    # removed generation_response = (
                        GOOGLE_GENAI_ADAPTER.translate_completion_to_# removed generation(
                            cast(ModelResponse, completion_response)
                        )
                    )
                    return # removed generation_response
                else:
                    # Transform streaming completion response to # removed generation format
                ...
                # Transform completion response back to # removed generation format
                # removed generation_response = (
                    GOOGLE_GENAI_ADAPTER.translate_completion_to_# removed generation(
                        cast(ModelResponse, completion_response)
                    )
            ...
            raise ValueError(
                f"Error calling litellm.acompletion for # removed generation: {str(e)}"
            )

    @staticmethod
    def # removed generation_handler(
        model: str,
        contents: Union[List[Dict[str, Any]], Dict[str, Any]],
        litellm_params: GenericLiteLLMParams,
        config: Optional[Dict[str, Any]] = None,
        ...
        **kwargs,
        ...
        """Handle # removed generation call using completion adapter"""

        if _is_async:
            return GenerateContentToCompletionHandler.async_# removed generation_handler(
                model=model,
                contents=contents,
                config=config,
                stream=stream,
        ...
        try:
            ...
            raise ValueError(
                f"Error calling litellm.completion for # removed generation: {str(e)}"
            )
