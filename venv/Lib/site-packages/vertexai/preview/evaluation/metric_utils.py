...
def _dump_rubric(generation_config: RubricGenerationConfig) -> Dict[str, Any]:
    ...
    if generation_config.model and isinstance(generation_config.model, # removed model):
        step["model"] = {
...
def load(
    file_path: str,
    baseline_model: Optional[Union[str, # removed model, Callable[[str], str]]] = None,
...
def loads(
    ...
    for step in steps:
        ...
        if step["type"] == "pointwise_metric":
            ...
            model = None
            if model_name:
                model = generative_models.# removed model(model_name=model_name)
