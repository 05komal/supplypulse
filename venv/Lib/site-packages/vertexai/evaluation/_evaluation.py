...
def _# removed generation_text_response(
    model: generative_models.# removed model, prompt: str, max_retries: int = 3
) -> str:
    ...
    for retry_attempt in range(max_retries):
        try:
            response = model.# removed generation(prompt)
...
def _# removed generation_text_response_genai(
        ...
        with futures.ThreadPoolExecutor(max_workers=constants.MAX_WORKERS) as executor:
            for _, row in df.iterrows():
                task = executor.submit(
                    _# removed generation_text_response_genai,
...
def _generate_responses_from_gemini_model(
    model: generative_models.# removed model,
    evaluation_run_config: evaluation_base.EvaluationRunConfig,
    is_baseline_model: bool = False,
) -> None:
    ...
    with tqdm(total=len(df)) as pbar:
        with futures.ThreadPoolExecutor(max_workers=constants.MAX_WORKERS) as executor:
            for _, row in df.iterrows():
                task = executor.submit(
                    _# removed generation_text_response,
                    prompt=row[constants.Dataset.PROMPT_COLUMN],
                    model=model,
                )
                task.add_done_callback(lambda _: pbar.update(1))
...
def _run_model_inference(
    model: Union[str, generative_models.# removed model, Callable[[str], str]],
    evaluation_run_config: evaluation_base.EvaluationRunConfig,
    ...
    if model:
        if response_column_name not in evaluation_run_config.metric_column_mapping:
            if constants.Dataset.PROMPT_COLUMN in evaluation_run_config.dataset.columns:
                ...
                if isinstance(model, generative_models.# removed model):
                    _LOGGER.warning(
                        "vertexai.generative_models.# removed model is deprecated for "
...
def evaluate(
    ...
    *,
    model: Optional[
        Union[str, generative_models.# removed model, Callable[[str], str]]
    ] = None,
    ...
    Args:
      ...
      model: The # removed model instance or a custom model function to generate
