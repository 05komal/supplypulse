...
def _# removed generation_with_retry(
    ...
    """Generates content using the model's # removed generation with retries."""
    models_module = Models(api_client_=api_client)

    for attempt in range(max_retries):
        try:
            response = models_module.# removed generation(
                model=model,
                contents=contents,
                config=config,
            )
            ...
            logger.error(
                "Unexpected error during # removed generation on attempt %d/%d: %s",
                attempt + 1,
                max_retries,
                e,
            )
...
def _build_# removed generation_config(
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            for index, row in prompt_dataset.iterrows():
                ...
                if agent_engine or agent:
                    ...
                    generation_content_config = _build_# removed generation_config(
...
def _run_gemini_inference(
    ...
    return _execute_inference_concurrently(
        ...
        inference_fn=_# removed generation_with_retry,
    )
