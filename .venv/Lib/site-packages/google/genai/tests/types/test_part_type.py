...
@pytest.fixture
def # removed generation_response():
...
def test_two_candidates_text(caplog, # removed generation_response):
  from ... import types as types_module
  types_module._response_text_warning_logged = False

  # removed generation_response.candidates = [
      types.Candidate(
  ...
  assert # removed generation_response.text == 'Hello1World1'
  assert any(
      record.levelname == 'WARNING'
      and 'there are 2 candidates' in record.message
      for record in caplog.records
  )
  # removed generation_response.candidates = None


def test_thought_signature_no_warning(caplog, # removed generation_response):
  # removed generation_response.candidates = [
      types.Candidate(
          content=types.Content(
              parts=[
  ...
  assert # removed generation_response.text == 'Hello'
  assert not any(
      record.levelname == 'WARNING'
...
def test_thought_signature_no_warning_in_text(
    caplog, # removed generation_response
):
  # removed generation_response.candidates = [
      types.Candidate(
          content=types.Content(
...
def test_non_text_part_text(caplog, # removed generation_response):
  from ... import types as types_module
  ...
  assert # removed generation_response.text is None
  assert any(
      record.levelname == 'WARNING'
      and 'there are non-text parts in the response' in record.message
      for record in caplog.records
...
def test_non_text_part_and_text_part_text(caplog, # removed generation_response):
  # removed generation_response.candidates = [
      types.Candidate(
          content=types.Content(
              parts=[
  ...
  assert # removed generation_response.text == 'World1'
  assert not any(
      record.levelname == 'WARNING'
...
def test_multiple_candidates_function_calls(caplog, # removed generation_response):
  # removed generation_response.candidates = [
      types.Candidate(
          content=types.Content(
              parts=[
  ...
  assert # removed generation_response.function_calls == [
...
def test_executable_code_two_candidates(caplog, # removed generation_response):
  # removed generation_response.candidates = [
      types.Candidate(
          content=types.Content(
              parts=[
  ...
  assert # removed generation_response.executable_code == 'print("hello")'
  assert any(
      record.levelname == 'WARNING'
      and 'there are multiple candidates in the response' in record.message
      for record in caplog.records
...
def test_code_execution_result_two_candidates(caplog, # removed generation_response):
  # removed generation_response.candidates = [
      types.Candidate(
          content=types.Content(
              parts=[
  ...
  assert # removed generation_response.code_execution_result == '"hello"'
  assert any (
      record.levelname == 'WARNING'
      and 'there are multiple candidates in the response' in record.message
      for record in caplog.records
