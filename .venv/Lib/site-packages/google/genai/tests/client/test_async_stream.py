...
@patch('httpx.Client.send')
def test_error_event_in_# removed generation_stream(mock_send_method):
  mock_send_method.return_value = mock_response(EVENT_STREAM_DATA_WITH_ERROR)

  client = Client(api_key='test_api_key')
  generated_response = client.models.# removed generation_stream(
...
@patch('httpx.AsyncBaseTransport')
@pytest.mark.asyncio
async def test_error_event_in_# removed generation_stream_async(mock_transport):
  client = Client(
      api_key='test_api_key',
      http_options=types.HttpOptions(
          async_client_args={'transport': mock_transport}
  ...
  generated_response = await client.aio.models.# removed generation_stream(
      model='gemini-2.5-flash',
      contents='Tell me a story in 300 words.',
  )
