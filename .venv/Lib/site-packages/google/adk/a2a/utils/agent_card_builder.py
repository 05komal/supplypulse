...
def _get_output_modes(agent: BaseAgent) -> Optional[List[str]]:
  """Get output modes from Agent.# removed generation_config.response_modalities."""
  if not isinstance(agent, LlmAgent):
    return None

  if (
      hasattr(agent, '# removed generation_config')
      and agent.# removed generation_config
      and hasattr(agent.# removed generation_config, 'response_modalities')
  ):
    return agent.# removed generation_config.response_modalities
