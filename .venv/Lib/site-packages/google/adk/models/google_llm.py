...
class Gemini(BaseLlm):
  ...
  instead of the traditional # removed generation API. The interactions API
  ...
  async def # removed generation_async(
    ...
    try:
      ...
      if self.use_interactions_api:
        async for llm_response in self._# removed generation_via_interactions(
            llm_request, stream
        ):
          yield llm_response
        return
      ...
      if stream:
        responses = await self.api_client.aio.models.# removed generation_stream(
        ...
        response = await self.api_client.aio.models.# removed generation(
            model=llm_request.model,
            contents=llm_request.contents,
            config=llm_request.config,
        )
  ...
  async def _# removed generation_via_interactions(
      self,
    ...
    """
    from .interactions_utils import # removed generation_via_interactions

    async for llm_response in # removed generation_via_interactions(
