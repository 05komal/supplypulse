...
@contextmanager
def use_# removed generation_span(
    llm_request: LlmRequest,
    invocation_context: InvocationContext,
    model_response_event: Event,
) -> Iterator[Span | None]:
  """Context manager encompassing `# removed generation {model.name}` span.
  ...
  if (
    ...
    with _use_extra_# removed generation_attributes(common_attributes):
      yield
  else:
    with _use_native_# removed generation_span_stable_semconv(
...
async def use_inference_span(
  ...
  if (
    ...
    async with _use_native_# removed generation_span(
        llm_request=llm_request,
        common_attributes=common_attributes,
    ) as gc_span:
...
def _instrumented_with_opentelemetry_instrumentation_google_genai() -> bool:
  maybe_wrapped_function = Models.# removed generation
...
@contextmanager
def _use_extra_# removed generation_attributes(
    extra_attributes: Mapping[str, AttributeValue],
):
  try:
    from opentelemetry.instrumentation.google_genai import # removed generation_EXTRA_ATTRIBUTES_CONTEXT_KEY
  ...
  tok = otel_context.attach(
      otel_context.set_value(
          # removed generation_EXTRA_ATTRIBUTES_CONTEXT_KEY, extra_attributes
      )
  )
  try:
    yield
...
def _set_common_# removed generation_attributes(
    span: Span,
    llm_request: LlmRequest,
    common_attributes: Mapping[str, AttributeValue],
):
  span.set_attribute(GEN_AI_OPERATION_NAME, '# removed generation')
  span.set_attribute(GEN_AI_REQUEST_MODEL, llm_request.model or '')
  span.set_attributes(common_attributes)


@contextmanager
def _use_native_# removed generation_span_stable_semconv(
    llm_request: LlmRequest,
    common_attributes: Mapping[str, AttributeValue],
) -> Iterator[GenerateContentSpan]:
  with tracer.start_as_current_span(
      f"# removed generation {llm_request.model or ''}"
  ) as span:
    span.set_attribute(GEN_AI_SYSTEM, _guess_gemini_system_name())
    _set_common_# removed generation_attributes(
...
@asynccontextmanager
async def _use_native_# removed generation_span(
    llm_request: LlmRequest,
    common_attributes: Mapping[str, AttributeValue],
...
class GenerateContentSpan:
  """Manages tracing within a `# removed generation` OpenTelemetry span.
...
def trace_# removed generation_result(span: Span | None, llm_response: LlmResponse):
  """Trace result of the inference in # removed generation span."""
