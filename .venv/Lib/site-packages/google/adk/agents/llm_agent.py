...
class LlmAgent(BaseAgent):
  ...
  **Examples:**
  ...
  # removed generation_config: Optional[types.GenerateContentConfig] = None
  ...
  @field_validator('# removed generation_config', mode='after')
  @classmethod
  def validate_# removed generation_config(
      cls, # removed generation_config: Optional[types.GenerateContentConfig]
  ) -> types.GenerateContentConfig:
    if not # removed generation_config:
      return types.GenerateContentConfig()
    if # removed generation_config.tools:
      raise ValueError('All tools must be set via LlmAgent.tools.')
    if # removed generation_config.system_instruction:
      raise ValueError(
          'System instruction must be set via LlmAgent.instruction.'
      )
    if # removed generation_config.response_schema:
      raise ValueError(
          'Response schema must be set via LlmAgent.output_schema.'
      )
    return # removed generation_config
  ...
  def model_post_init(self, __context: Any) -> None:
    ...
    if getattr(
        self.# removed generation_config, 'thinking_config', None
    ) and getattr(self.planner, 'thinking_config', None):
      warnings.warn(
          'Both `thinking_config` in `# removed generation_config` and a '
  ...
  def _parse_config(
    ...
    if config.# removed generation_config:
      kwargs['# removed generation_config'] = config.# removed generation_config
